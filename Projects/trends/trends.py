...
def extract_words(text):
    ...
    for i in range(len(text)):
        if text[i] in ascii_letters or text[i] == ' ':
            text = text
        else:
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    "*** YOUR CODE HERE ***"
    comment = tweet_words(tweet)
    sentiments = [get_word_sentiment(i) for i in comment]
    sentiment_values = [sentiment_value(x) for x in sentiments if has_sentiment(x)]
    if sentiment_values == []:
        return make_sentiment(None)
    return make_sentiment(sum(sentiment_values)/len(sentiment_values))
...
def find_centroid(polygon):
    ...
    total = 0
    while i < len(polygon) - 1:
        total = total + (x_c[i] + x_c[i+1])*(x_c[i]*y_c[i+1] - x_c[i+1]*y_c[i])
        i += 1
    Cx = abs(total/(6*area))
    i = 0
    total = 0
    while i < len(polygon) - 1:
        total = total + (y_c[i] + y_c[i+1])*(x_c[i]*y_c[i+1] - x_c[i+1]*y_c[i])
        i += 1
    Cy = abs(total/(6*area))
    return [Cx, Cy, area]
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    centroids = [find_centroid(x) for x in polygons] #returns list of [Cx,Cy, area]'s
    i = 0
    Cxa = 0
    Cya = 0
    a = 0
    while i < len(polygons):
        Cxa += centroids[i][0]*centroids[i][2]
        Cya += centroids[i][1]*centroids[i][2]*-1
        a += centroids[i][2]
        i += 1
    return make_position(Cxa/a,Cya/a)
