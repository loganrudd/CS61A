...
class VendingMachine(object):
    ...
    def vend(self):
        if self.amount > self.cost and self.stock > 0 and self.amount - self.cost > 0:
            self.stock -= 1
            balance = self.amount - self.cost
            self.amount = 0
            return 'Here is your ' + self.item + ' and $' + str(balance) + ' change.'
        elif self.amount == self.cost and self.stock > 0:
            ...
            return 'You must deposit $' + str(10 - self.amount) + ' more.'
...
class MissManners(object):
    ...
    def ask(self, message, *args):
        if message[0:6] == 'please':
            if hasattr(self.object,message[7:]):
                return getattr(self.object,message[7:])(*args)
            return 'Thanks for asking, but I know not how to' + message[6:]
        return 'You must learn to say please first.'
#getattr: takes in object and string and it retreives that method(string)
# hasattr:takes an object and string and sees if the object has that string as an attribute
##########################################
#           Challenge Problem            #
